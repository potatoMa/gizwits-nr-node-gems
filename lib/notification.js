...
module.exports = function(RED) {
  function utc2beijing(utc_datetime) {
    ...
    // 时间戳转为时间
    var beijing_datetime = new Date(parseInt(timestamp) * 1000).toLocaleString().replace(/年|月/g, "-").replace(/日/g, " ").replace(/\//g, "-").replace(/上午|下午/g, "");
    ...
    _this.on('input', async function(msg) {
      ...
      try {
        ...
        if (props.notiType === 'phone') {
          ...
          if (props.TXT_32.length > 20) {
            _this.send({
              payload: 'TXT_32变量长度不得超过20'
            });
            return;
          }
          var vars = {
          ...
          var options = {
            ...
            data: JSON.stringify({
              ...
              vars: {
                ...vars,
                TIME : utc2beijing(now.toISOString()),
              }
            })
          };
          ...
          var options = {
            ...
            data: JSON.stringify({
              ...
              text: `【机智云】app提醒您，${props.deviceName}的${props.eventName}在${utc2beijing(now.toISOString())}被执行。`
            })
          };
